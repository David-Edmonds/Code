...
The Job Opportunity Engine can prepare an import-ready CSV for a private Airtable review table. The repository does not contain an Airtable base ID, table ID, API key, actual application, recruiter message, or private account data.

## Private table fields

A compatible private tracker can support:
...
1. Open the private review table.
...
Use only an approved, narrowly scoped source such as a specific Gmail job-alert search, a CSV export, or manually supplied URLs. Deduplicate before importing, keep private source material outside GitHub, and avoid recurring polling until Airtable API and automation usage has been measured.
